...
func NewDefaultGroupLoader(client client.Client, eventRecorder record.EventRecorder, annotationParser annotations.Parser) *defaultGroupLoader {
...
func (m *defaultGroupLoader) Load(ctx context.Context, groupID GroupID) (Group, error) {
	...
	var members []ClassifiedService
	var inactiveMembers []*corev1.Service
	for index := range svcList.Items {
		svc := &svcList.Items[index]
		membershipType, classifiedIng, err := m.checkGroupMembershipType(ctx, groupID, svc)
		if err != nil {
			return Group{}, errors.Wrapf(err, "Service: %v", k8s.NamespacedName(svc))
		}
...
func (m *defaultGroupLoader) checkGroupMembershipType(ctx context.Context, groupID GroupID, ing *corev1.Service) (groupMembershipType, ClassifiedService, error) {
